...
public class ReferenceCallsAction extends AbstractCanceableAction
{
    private static final long serialVersionUID = -8714115023152749543L;
    protected static ReferenceCallsAction m_inst = null;
    protected int m_refs;
    protected int m_curRefs;

    public static ReferenceCallsAction getInstance(MainFrame mainFrame)
    {
        if (m_inst == null)
        {
            m_inst = new ReferenceCallsAction("Print calls to references", null);
            m_inst.setMainFrame(mainFrame);
        }
        return m_inst;
    }

    protected ReferenceCallsAction(String arg0, Icon arg1)
    {
        super(arg0, arg1);
    }
                ...
                for (int j = 0; j < callerInvokations.size(); j++)
                {
                            ...
                            callerInv.invDescriptor.equals(called.getMethod().getDescriptor()))
                    {
                        ...
                        if (caller.getContext() instanceof ApkClassContext) {
                            ...
                            injection =
                                    new DalvikMethodOffsetLog(Util.getMethodSignature(caller.getMethod()),
    ...
    private void addInvokations(List<Reference> res, Iterator<Reference> refs)
    {
