...
import andreflect.injection.impl.DalvikMethodOffsetInvokeReturn;

public class DalvikReferenceCallsReturnAction extends AbstractCanceableAction
{
    private static final long serialVersionUID = -334735447277492453L;
    protected static DalvikReferenceCallsReturnAction m_inst = null;
    protected int m_refs;
    protected int m_curRefs;

    public static DalvikReferenceCallsReturnAction getInstance(MainFrame mainFrame)
    {
        if (m_inst == null)
        {
            m_inst = new DalvikReferenceCallsReturnAction("Print calls to references(with return value)", null);
            m_inst.setMainFrame(mainFrame);
        }
        return m_inst;
    }

    protected DalvikReferenceCallsReturnAction(String arg0, Icon arg1)
    {
        super(arg0, arg1);
    }
                ...
                for (int j = 0; j < callerInvokations.size(); j++)
                {
                            ...
                            callerInv.invDescriptor.equals(called.getMethod().getDescriptor()))
                    {
                        ...
                        if (caller.getContext() instanceof ApkClassContext) {
                            ...
                            DalvikMethodOffsetInvokeReturn dalvikinjection =
                                    new DalvikMethodOffsetInvokeReturn(Util.getMethodSignature(caller.getMethod()),
                                            callerInv.offsetIns, output);
                            Instruction nextIns = ((DexMethod) (caller.getMethod())).getNextInstruction(callerInv.offsetIns);
                            if (nextIns.opcode == Opcode.MOVE_RESULT
                                    || nextIns.opcode == Opcode.MOVE_RESULT_OBJECT
                                    || nextIns.opcode == Opcode.MOVE_RESULT_WIDE) {
                                dalvikinjection.setMoveResultIns(nextIns);
                                nextIns = ((DexMethod) (caller.getMethod())).getNextInstruction(nextIns);
                            } else {
                                dalvikinjection.setMoveResultIns(null);
                            }
                            dalvikinjection.setOffsetInstruction(nextIns);
                            dalvikinjection.setInvokeIns(callerInv.offsetIns);
